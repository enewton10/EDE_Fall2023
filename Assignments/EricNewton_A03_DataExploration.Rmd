...
```{r Summary of Species Common Name}
...
```{r Frequency Plot of Neonics Publication Year}
ggplot(Neonics) + 
...
```{r Frequency Plot with Test Location Highlighted}
ggplot(Neonics) + 
...
```{r Bar Plot of Neonics Endpoints}
ggplot(Neonics) +
...
```{r Class of collectDate from Factor to Date}
...
August_2018 <- unique(Litter$collectDate, incomperables = FALSE) #unique function tells you the number of levels in the factor, and the values within.
August_2018
...
```{r Bar Graph of Functional Group Count}
ggplot(Litter, aes(x = functionalGroup)) + geom_bar() #geom_bar provides a bar plot, which shows the count of values in functionalGroup.
```
